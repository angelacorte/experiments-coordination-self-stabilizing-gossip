...
@PublishedApi
internal data class Replica<Type>(
    val id: ReplicaID,
    val process: () -> Type,
)
 ...
 */
@PublishedApi
internal data class TimedReplica(
    val id: ReplicaID,
    val remainingTimeToLive: Duration,
)
...
inline fun <reified Type : Any?> Aggregate<*>.timeReplicated(
    ...
    evolving(emptyList<Replica<Type>>()) { localReplicas ->
        // the replica ID is either the id of the new replica to spawn or null if nothing new should spawn
        val baseReplicas =
            when (val newReplicaId = newReplicaID(timeToSpawn, currentTime)) {
                null -> localReplicas
 ...
 */
@PublishedApi
internal fun Aggregate<*>.newReplicaID(
    timeToLive: Duration,
    currentTime: Instant,
): ReplicaID? {
    ...
    return sharing(TimedReplica(0UL, ZERO)) { replicas ->
        ...
        when {
            ...
            replicas.local.value.remainingTimeToLive <= ZERO -> { // I have to create a new replica
                ...
                TimedReplica(replicas.local.value.id, timeLeft)
